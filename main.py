...
class MyResults(Results):
    def __init__(self,yolo2_img, orig_img, path, names, boxes=None, masks=None, probs=None, keypoints=None, obb=None) -> None:
        super().__init__(orig_img, path, names, boxes, masks, probs, keypoints, obb)
        self.yolo2_img = yolo2_img
    def plot(
        self,
        conf=True,
        line_width=None,
        ...
        if pred_boxes is not None and show_boxes:
            for d in reversed(pred_boxes):
                ...
                # 新增：截取包含指定标签的框内容
                # print(names[c])
                if names[c] == "no protective cap":
                    x1, y1, x2, y2 = map(int, box)  # 假设box是[x1, y1, x2, y2]格式
                    x0,y0 = self.orig_img.shape[1],self.orig_img.shape[0]
                    # print(x1, y1, x2, y2)
                    # print(self.orig_img)
                    # print(type(self.orig_img))
                    cropped_img = self.orig_img[
                        y1:
                        y0 if y1+910 > y0 else y1+910, 
                        x1-85 if x1-85>0 else 0 : 
                        x0 if x1+165 >x0 else x1+165
                        ]
                    # print(cropped_img.shape)
                    # Image.fromarray(cv2.cvtColor(cropped_img, cv2.COLOR_BGR2RGB)).save("2.jpg")
                    # self.set_value(cv2.cvtColor(cropped_img, cv2.COLOR_BGR2RGB))
                    self.yolo2_img.emit(cropped_img)
                    
                elif names[c] == "no protective clothing":
                    x1, y1, x2, y2 = map(int, box)
                    x0,y0 = self.orig_img.shape[1],self.orig_img.shape[0]
                    cropped_img = self.orig_img[
                        y1-140 if y1 - 140 > 0 else y0:
                        y0 if y1+770 > y0 else y1+770, 
                        x1 : 
                        x0 if x1+250 >x0 else x1+250
                        ]
                    # extracted_images.append((names[c], cropped_img))
                    # print(cropped_img.shape)
                    self.yolo2_img.emit(cropped_img)
...
class YoloPredictor(BasePredictor, QObject):
                    ...
                    if self.continue_dtc:
                        ...
                        with profilers[2]:
                            if self.task == "Classify":
                                ...
                                # print(im0s)
                                # print(self.results)
                                # print(self.results[0].names)
                                # print(self.results[0].orig_img.shape)
                                # Image.fromarray(self.results[0].orig_img).save("1.jpg")
                                # cv2.imwrite('2.jpg',cv2.cvtColor(self.results[0].orig_img, cv2.COLOR_BGR2RGB))
                            elif self.task == "Pose":
                        ...
                        for i in range(n):
                            ...
                            # 发送测试结果
                            # Image.fromarray(cv2.cvtColor(im0, cv2.COLOR_BGR2RGB)).save("1.jpg")
    ...
    def postprocess(self, preds, img, orig_img):
        ...
        for i, pred in enumerate(preds):
            ...
            results.append(
                MyResults(yolo2_img=self.yolo2_img,
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # self.image_gallery.add_image("2.jpg")
        # 初始页面
        self.task = "Detect"
        self.PageIndex = 1
        ...
        self.yolo_predict.yolo2_img.connect(
            lambda x: self.conditional_add_image(x)
        )
        # self.yolo_predict.yolo2_img.connect(
        #     lambda x: self.image_gallery.add_image(x)
        # )
    ...
    def a_value_changed(self, new_value):
            self.image_gallery.add_image(new_value)
    ...
    # 定义一个函数来处理计数和条件判断
    def conditional_add_image(self,x):
        if self.yolo_predict.count == 0 or self.yolo_predict.count % 30 == 0:
            self.image_gallery.add_image(x)
        if self.yolo_predict.count > 3000:
            self.yolo_predict.count=0
        self.yolo_predict.count += 1
